...
[ -d "$dp0/build" ] || cmake -S "$dp0" -B "$dp0/build" -DCMAKE_BUILD_TYPE=Release -G Ninja -DSTBIR_SSE2:BOOL=ON -DSTBIR_AVX:BOOL=ON -DSTBIR_AVX2:BOOL=ON -DBUILD_SQUISH_WITH_SSE2:BOOL=ON
if [ -d "$dp0/build" ] ; then
